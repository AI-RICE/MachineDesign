...
2. For each worker, copy the base `.aedt` project file to its own file
   before opening it. Ansys locks a project file while it is open, so every
   worker needs its own copy rather than sharing one file.
...
4. Inside a worker, solve each task with `analyze_setup(setup_name,
   use_auto_settings=False, cores=1, tasks=1)`, pinning each solve to a
   single core. Several workers are already running concurrently, so
   letting each one grab multiple cores would oversubscribe the machine.
