...
torque waveforms match at each step. The second iteration specifically exercises the
delete-then-reassign motion-setup path (the first iteration only ever sees an empty
motion setup, since nothing has been assigned yet). Run explicitly with:
...
def test_legacy_and_live_design_match_across_rotor_rebuilds(tmp_path):
    ...
    try:
        ...
        for seed, barriers in zip(SEEDS, barrier_sets):
            torque_legacy = _compute_torque(legacy_design, barriers, _add_rotor_legacy)
            torque_live = _compute_torque(live_design, barriers, _add_rotor_live)
