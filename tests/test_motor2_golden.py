...
golden_tor = np.loadtxt(os.path.join(os.path.dirname(__file__), "golden", "motor2_baseline", "tor2.csv"), delimiter=",", skiprows=1)
...
def test_torque_matches_golden_baseline(tmp_path):
    ...
    try:
        ...
        np.testing.assert_allclose(Tor, golden_tor, rtol=0.03, err_msg="torque waveform drifted from the golden baseline")
    finally:
